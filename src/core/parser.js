...
class ImageParser {
    ...
    // --- ComfyUI Workflow Static Helpers ---

    static findNodeById(nodes, nodeId) {
        return nodes.find(node => node.id === nodeId) || null;
    }

    static findSourceNodeForLink(nodes, targetLinkId) {
    ...
    static findTextEncodeNodeForConditioning(initialLinkId, nodes) {
        let currentLinkId = initialLinkId;
        let safetyCounter = 0;

        while (currentLinkId !== null && safetyCounter < 20) {
            const sourceInfo = ImageParser.findSourceNodeForLink(nodes, currentLinkId);
            ...
            if (sourceNode.inputs) {
                const relevantInput = sourceNode.inputs.find(input =>
                    ...
                    sourceNode.outputs[sourceInfo.outputIndex]?.name === input.name
                );

                if (relevantInput && relevantInput.link) {
                    currentLinkId = relevantInput.link;
                } else {
    ...
    static parseWorkflow(workflowData) {
        ...
        if (positiveInput && positiveInput.link) {
            const positiveTextNode = ImageParser.findTextEncodeNodeForConditioning(positiveInput.link, nodes);
        ...
        if (negativeInput && negativeInput.link) {
            const negativeTextNode = ImageParser.findTextEncodeNodeForConditioning(negativeInput.link, nodes);
